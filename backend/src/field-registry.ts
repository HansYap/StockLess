import type {
  CanonicalField,
  FieldDefinition,
} from "./contracts.ts";

export const FIELD_REGISTRY: Readonly<Record<CanonicalField, FieldDefinition>> = {
  transaction_date: {
    field: "transaction_date",
    label: "Transaction date",
    description: "The calendar date on which the sale or return was recorded.",
    status: "core",
    valueKind: "date",
    grain: "transaction",
    unlocks: ["Weekly product history", "Timeline-gap evidence", "Recent weekly average"],
    aliases: ["transaction date", "sale date", "sales date", "transaction datetime", "txn date", "txndate", "sold at", "tarikh jualan"],
    acquisitionSource: "file",
  },
  product_code: {
    field: "product_code",
    label: "Product code, SKU or barcode",
    description: "A stable identifier that keeps each product separate.",
    status: "conditional_core",
    valueKind: "text",
    grain: "product",
    unlocks: ["Preferred stable product identity", "Weekly product history"],
    aliases: ["product code", "sku", "barcode", "item code", "stock code", "product id", "plu", "kod produk"],
    acquisitionSource: "file",
  },
  product_name: {
    field: "product_name",
    label: "Product name",
    description: "A readable product label and one part of composite identity.",
    status: "conditional_core",
    valueKind: "text",
    grain: "product",
    unlocks: ["Readable product labels", "Composite product identity"],
    aliases: ["product name", "item name", "product", "item", "description", "nama produk"],
    acquisitionSource: "file",
  },
  pack_variant: {
    field: "pack_variant",
    label: "Pack variant or unit",
    description: "The pack size or unit needed to prevent unlike products being merged.",
    status: "conditional_core",
    valueKind: "text",
    grain: "product",
    unlocks: ["Composite product identity", "Pack-size separation"],
    aliases: ["pack variant", "variant", "unit", "uom", "size", "pack", "unit size"],
    acquisitionSource: "file",
  },
  quantity_sold: {
    field: "quantity_sold",
    label: "Quantity sold",
    description: "The sale or return quantity recorded for the transaction.",
    status: "core",
    valueKind: "decimal",
    grain: "transaction",
    unlocks: ["Net weekly demand", "Returns and adjustments evidence"],
    aliases: ["quantity sold", "qty sold", "units sold", "sold quantity", "sales qty", "quantity", "qty", "pieces sold", "kuantiti jualan"],
    acquisitionSource: "file",
  },
  current_stock: {
    field: "current_stock",
    label: "Current stock",
    description: "The current product-level stock snapshot; repeated values are not summed.",
    status: "feature_dependent",
    valueKind: "non_negative_decimal",
    grain: "product",
    unlocks: ["Stock freshness", "Descriptive weeks of cover"],
    aliases: ["current stock", "stock on hand", "soh", "closing stock", "current inventory", "inventory qty", "remaining units", "baki stok"],
    acquisitionSource: "either",
  },
  stock_as_of_date: {
    field: "stock_as_of_date",
    label: "Stock as-of date",
    description: "The date on which the current-stock snapshot was measured.",
    status: "feature_dependent",
    valueKind: "date",
    grain: "product",
    unlocks: ["Stock freshness", "Descriptive weeks of cover"],
    aliases: ["stock as of date", "stock date", "snapshot date", "inventory date", "count date", "stock count date", "stock take day", "inventory as at", "inventoryasat", "tarikh stok"],
    acquisitionSource: "either",
  },
  planned_order_quantity: {
    field: "planned_order_quantity",
    label: "Planned order quantity",
    description: "A planned purchase quantity for a later purchase audit.",
    status: "later_locked",
    valueKind: "non_negative_decimal",
    grain: "order",
    unlocks: ["Iteration 2 purchase audit"],
    aliases: ["planned order quantity", "planned order", "order qty", "purchase quantity"],
    acquisitionSource: "either",
  },
  incoming_stock_quantity: {
    field: "incoming_stock_quantity",
    label: "Incoming stock quantity",
    description: "Stock already expected to arrive.",
    status: "later_locked",
    valueKind: "non_negative_decimal",
    grain: "inbound",
    unlocks: ["Iteration 2 on-order position", "Purchase audit"],
    aliases: ["incoming stock quantity", "incoming quantity", "on order", "inbound qty"],
    acquisitionSource: "either",
  },
  incoming_stock_expected_date: {
    field: "incoming_stock_expected_date",
    label: "Incoming stock expected date",
    description: "The expected arrival date of incoming stock.",
    status: "later_locked",
    valueKind: "date",
    grain: "inbound",
    unlocks: ["Iteration 2 arrival timing"],
    aliases: ["incoming stock expected date", "expected arrival date", "eta", "delivery date"],
    acquisitionSource: "either",
  },
  expiry_date: {
    field: "expiry_date",
    label: "Expiry date",
    description: "The expiry date of a product lot.",
    status: "later_locked",
    valueKind: "date",
    grain: "lot",
    unlocks: ["Iteration 2 expiry-aware note"],
    aliases: ["expiry date", "expiration date", "best before", "use by"],
    acquisitionSource: "either",
  },
  expiry_quantity: {
    field: "expiry_quantity",
    label: "Expiry quantity",
    description: "The quantity affected by a product-lot expiry date.",
    status: "later_locked",
    valueKind: "non_negative_decimal",
    grain: "lot",
    unlocks: ["Iteration 2 expiry-aware note"],
    aliases: ["expiry quantity", "expiring quantity", "expiry qty"],
    acquisitionSource: "either",
  },
  supplier_id_or_name: {
    field: "supplier_id_or_name",
    label: "Supplier ID or name",
    description: "The supplier responsible for the product.",
    status: "later_locked",
    valueKind: "text",
    grain: "supplier",
    unlocks: ["Iteration 2 supplier scenario"],
    aliases: ["supplier", "supplier id", "supplier name", "vendor", "vendor id"],
    acquisitionSource: "either",
  },
  supplier_lead_time_days: {
    field: "supplier_lead_time_days",
    label: "Supplier lead time in days",
    description: "The number of days a supplier usually needs to deliver.",
    status: "later_locked",
    valueKind: "non_negative_integer",
    grain: "supplier",
    unlocks: ["Iteration 2 reorder timing", "Supplier scenario"],
    aliases: ["supplier lead time days", "lead time days", "lead time"],
    acquisitionSource: "either",
  },
  pack_size: {
    field: "pack_size",
    label: "Pack size",
    description: "A numeric or well-defined pack size used for later pack rounding.",
    status: "later_locked",
    valueKind: "text",
    grain: "product",
    unlocks: ["Iteration 2 pack rounding", "Product identity interpretation"],
    aliases: ["pack size", "case size", "carton size", "units per pack"],
    acquisitionSource: "either",
  },
};

export const CANONICAL_FIELDS = Object.freeze(Object.keys(FIELD_REGISTRY) as CanonicalField[]);

export const CORE_COLUMN_PATHS = Object.freeze([
  {
    id: "stable" as const,
    label: "Stable product identifier",
    requiredFields: ["transaction_date", "product_code", "quantity_sold"] as const,
  },
  {
    id: "composite" as const,
    label: "Confirmed product name and pack variant",
    requiredFields: ["transaction_date", "product_name", "pack_variant", "quantity_sold"] as const,
  },
]);

export const UPLOAD_REQUIREMENTS = Object.freeze({
  maxBytes: 10 * 1024 * 1024,
  maxRows: 100_000,
  supportedExtension: ".csv",
  supportedDelimiters: [",", ";", "\t"] as const,
  coreDescription:
    "StockLess needs a transaction date, quantity sold, and either a stable product code or a confirmed product name plus pack variant.",
});

export const PRIVACY_NOTICE = Object.freeze({
  beforeUpload:
    "Your raw CSV rows are processed in this browser and no account is required. StockLess may download local AI model assets, but your headings, preview values, rows, derived series, and product identifiers are not uploaded to an AI or API service.",
  persistence:
    "Unsaved retailer data is kept in memory only. Clearing or closing the session removes application references; JavaScript cannot claim a physical RAM overwrite.",
});
